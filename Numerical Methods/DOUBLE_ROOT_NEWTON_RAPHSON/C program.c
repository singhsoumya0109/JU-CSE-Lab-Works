...
double newton_raphson(double first,int num_iteration)
{
	...
	{
	second=first-(2*(function(first))/function1(first)); //finding out the next approx roots
	absolute_error1=second-first; //to find the absolute error
	if(fabs(absolute_error1)<=0.00005)
		break;
	 order=fabs(log(fabs(absolute_error1))/log(fabs(absolute_error0))); //calculating the order of convergence
    absolute_error0=absolute_error1;
	fprintf(data,"%2d  %.4lf  %.4lf  %.4lf  %.4lf\n",index+1,first,function(first),fabs(absolute_error1),order); //writing the data in the file
	first=second; //second is initialised to first
